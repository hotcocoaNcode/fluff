...
public class Main {
    public static void main(String[] args) {
        ...
        boolean readBytecode = Arrays.stream(args).toList().contains("-Mread");

        boolean interpretMode = Arrays.stream(args).toList().contains("-Minterpret");
        ...
        if (interpreterV1){
            JoshLogger.error("Fluff V1 is no longer supported." +
                                       "\nPlease update your codebase to support the latest syntax and features.");
            // About damn time I remove this.
            /*
        ...
        FluffCompiler bytecodeCompiler = new FluffCompiler(bytecodeRevision, cexes);
        ...
        JoshLogger.importantNormal("JavaFluff, version " + VersionInfo.getVersionString());
        ...
        JoshLogger.log("Bytecode Compiler instantiated with version " + bytecodeRevision);
